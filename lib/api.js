...
export async function toggleUserActiveApi(userId) {
    try {
...
export async function toggleAdminListingStock(listingId) {
    try {
        const { data } = await axios.put(`/api/admin/listings/${listingId}`)
        return data
    } catch (err) {
        throw new Error(err.response?.data?.error || 'Failed to toggle listing')
    }
}
